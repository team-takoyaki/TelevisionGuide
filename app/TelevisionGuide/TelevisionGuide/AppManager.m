...
#define REMEMBER_A_URL @"http://api.team-takoyaki.com/remember.php"
#define FORGET_A_URL @"http://api.team-takoyaki.com/forget_action.php"
...
- (void)requestRemember:(NSString *)pId
{
    NSString *urlString = [NSString stringWithFormat:@"%@?user_id=%@&pg_id=%@", REMEMBER_A_URL, _UUID, pId];
...
- (void)requestForget:(NSString *)pId
{
    NSString *urlString = [NSString stringWithFormat:@"%@?user_id=%@&pg_id=%@", FORGET_A_URL, _UUID, pId];
...
- (void)updateRememberWithTarget:(id)aTarget selector:(SEL)aSelector
{
    ...
    [manager GET:urlString parameters:nil success:^(AFHTTPRequestOperation *operation, id responseObject) {
        ...
        for (NSDictionary *dict in programs) {
            ...
            NSString *pImage = (NSString *)[dict objectForKey:@"pg_icon"];
            [program setProgramImage:pImage];
            ...
            NSString *sDate = (NSString *)[dict objectForKey:@"pg_day"];
            [program setProgramDay:sDate];
            
            NSString *pTime = (NSString *)[dict objectForKey:@"pg_time"];
            [program setProgramTime:pTime];
